...
class UserForm(BaseForm,ModelForm):
    ...
    nome_completo = StringField('Nome / Razao Social',validators=[validators.DataRequired()])
    celular = StringField('Celular',validators=[validators.DataRequired()])
    rg = StringField('RG',validators=[validators.DataRequired()])
    cpf_cnpj = StringField('CPF/CNPJ',validators=[validators.DataRequired(),Unique(User.cpf_cnpj,message=ITEM_JA_CADASTRADO.format('CPF CNPJ'))])
    roles = QuerySelectMultipleField( 
        label=u"Permissões", get_pk=lambda x: x.id, 
        query_factory=lambda: Role.query,
        get_label=lambda item: item.description ,
        render_kw={'select2':''} 
    )
